...
|        |       |      |      |
|  ----  | ----  |----  |----  |
| [数组](#数组)        | [链表](#链表)     | [哈希表](#哈希表)            | [字符串](#字符串) |
| [栈与队列](#栈与队列) | [二叉树](#二叉树) | [回溯算法](#回溯算法)        | [贪心算法](#贪心算法) |
| [动态规划](#动态规划) | [单调栈](#单调栈) | [并查集](#并查集) | [背包问题模板](#背包问题模板) |
| [岛屿问题](#岛屿问题) | [LeetCode热题100](#LeetCode热题100) | [常用 STL](#常用STL) |  |
...
4. [反转字符串中的单词](4.字符串/4.反转字符串中的单词.md) --- leetcode 151
...
26. [最长重复子数组](9.动态规划/26.最长重复子数组.md) --- leetcode 718
    - dp[i][j] 表示以nums[i]和nums[j]结尾的子数组, 最大相等的长度
    - 当nums[i]和nums[j]相等时, 长度为1+其坐上角dp[i-1][j-1]的大小, 需要知道上一个字符相等与否的情况
27. [最长公共子序列](9.动态规划/27.最长公共子序列.md) --- leetcode 1143
    - dp 表示text1[i]和text2[j]的最长公共子序列
28. [携带矿石资源](9.动态规划/28.携带矿石资源.md) --- KamaCoder 56
    - 多重背包
29. [购物单](9.动态规划/29.购物单.md) --- 牛客 HJ16
    - 有依赖的背包

## 单调栈
1. [每日温度](10.单调栈/1.每日温度.md) --- leetcode 739
    - 栈内存下标
2. [下一个更大元素Ⅰ](10.单调栈/2.下一个更大元素Ⅰ.md) --- leetcode 496 
3. [下一个更大元素Ⅱ](10.单调栈/3.下一个更大元素Ⅱ.md) --- leetcode 503
    - 拼接数组, 同上 单调栈
4. [接雨水](10.单调栈/4.接雨水.md) --- leetcode 42 
    - 按行走, 单调栈, 栈底到栈顶降序, 当前元素大于栈顶循环求水
    - 按列走, 双指针, 存储每一个位置左边的最大值和右边的最大值
        - 求右最大, 从后向前遍历dp[i] = max(dp[i+1], h[i+1])
        - 求左最大, 从前向后遍历dp[i] = max(dp[i-1], h[i-1])

## 并查集
1. [并查集模板](11.并查集/1.并查集模板.md)
2. [冗余连接](11.并查集/2.冗余连接.md) --- leetcode 684
3. [寻找图中是否存在路径](11.并查集/3.寻找图中是否存在路径.md) --- leetcode 1971
3. [连接所有点的最小费用](11.并查集/4.连接所有点的最小费用.md) --- leetcode 1584
    - 最小生成树, Kruskal算法 (从权重最小的边开始, 选择不连通的边)
    - 并查集检查连通性
...
5. [和为K的子数组](LeetCode热题100/5.和为K的子数组.md) --- leetcode 560 
    - 前缀和: 以当前下标为结尾的数组, 是否有和为 k 的连续子数组
    - 如果存在 sum[i] - k, 则++
    - 使用 map 记录 前缀和 的个数
6. [最小覆盖子串](LeetCode热题100/6.最小覆盖子串.md) --- leetcode 76 
7. [LRU 缓存](LeetCode热题100/7.LRU缓存.md) --- leetcode 146 
    - 双链表 + map

## 背包问题模板
1. [01背包](背包问题模板/1.01背包.md)
    - 用上一行更新本行
2. [完全背包](背包问题模板/2.完全背包.md)
    - 用本行更新本行
3. [多重背包](背包问题模板/3.多重背包.md)
    - 列叠加: 直接叠加 / 二进制叠加
    - 用上一行更新本行

## 岛屿问题
1. [岛屿问题模板](岛屿问题/1.岛屿问题模板.md) --- leetcode 200
    - 定义方向direction, 即从一个陆地出发, 能走的方向
    - 定义检查函数check, 判断该位置是否已经走出边界
    - 回溯搜索dfs
        - 判断是否位于界内
        ...
        - 循环, 根据方向向四周移动
    - 两重循环, 寻找陆地, 进行dfs
2. [岛屿数量](岛屿问题/2.岛屿数量.md) --- leetcode 200
    - 同模板
3. [岛屿的周长](岛屿问题/3.岛屿的周长.md) --- leetcode 463 
    - 遇到出界/海 +1, 遇到已走过的 0
4. [岛屿的最大面积](岛屿问题/4.岛屿的最大面积.md) --- leetcode 695 
    - 主函数每次dfs, 记录陆地数量
5. [最大人工岛](岛屿问题/5.最大人工岛.md) --- leetcode 827  
    - 两次遍历
...
## 常用STL
1. [algorithm](常用STL/1.algorithm.md)
2. [vector](常用STL/2.vector.md)
3. [set](常用STL/3.set.md)
4. [map](常用STL/4.map.md)
5. [priority_queue](常用STL/5.priority_queue.md)
6. [deque](常用STL/6.deque.md)